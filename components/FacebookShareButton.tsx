...
export default function FacebookShareButton({ 
  ...
  const handleShare = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    // Récupère le domaine actuel (ex: https://saidati.me)
    const currentDomain = window.location.origin;
    
    // Correspondance exacte avec votre structure de dossier : app/shop/product/[slug]
    const fullProductUrl = `${currentDomain}/shop/product/${slug}`;
    
    // URL officielle de partage Facebook corrigée
  ...
  return (
    <button
      onClick={handleShare}
      className="flex items-center justify-center gap-2 bg-[#1877F2] text-white text-xs font-bold px-4 py-2.5 rounded-xl hover:bg-[#166FE5] active:scale-95 transition-all shadow-md hover:shadow-lg cursor-pointer w-full sm:w-auto"
      dir="rtl"
    >
