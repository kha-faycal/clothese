...
export default function CartDrawer() {
 ...
 const handleCheckout = async (e: React.FormEvent) => {
  ...
  // 🟢 Déboguage : Affiche l'état des variables dans votre console pour voir laquelle est vide
  ...
  // Validation ultra-sécurisée avec suppression des espaces vides
  if (!customerName.trim()) {
    toast.error("يرجى كتابة الاسم الكامل");
    return;
  }
  if (!phone.trim()) {
    toast.error("يرجى كتابة رقم الهاتف");
    return;
  }
  ...
  if (!commune.trim()) {
    toast.error("يرجى كتابة البلدية أو العنوان");
    return;
  }
  ...
  <div className="fixed inset-0 z-50 flex justify-end" dir="rtl">
    ...
    <div className="relative w-full max-w-md bg-gray-950 text-white h-full shadow-2xl flex flex-col border-r border-gray-800 animate-in slide-in-from-right duration-300">
      ...
      {/* 🟢 الإصلاح الجذري: النموذج (Form) يغلف كامل محتوى السلة الداخلي */}
      <form onSubmit={handleCheckout} noValidate className="flex-1 flex flex-col overflow-hidden">
        
        {/* المحتوى القابل للتمرير (المنتجات + خانات الشحن) */}
        <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4">
          {items.length === 0 ? (
            ...
            <>
              ...
              {/* قسم معلومات الشحن */}
              <div className="mt-2 pt-4 border-t border-gray-900 flex flex-col gap-3">
                ...
                <input 
                  type="text" 
                  placeholder="البلدية / العنوان (Commune / Adresse)" 
                  value={commune} 
        ...
        {/* الجزء السفلي الثابت (الفاتورة وزر التأكيد) */}
        {items.length > 0 && (
          <div className="p-4 border-t border-gray-900 bg-gray-950 flex flex-col gap-2.5 mt-auto">
            ...
            <div className="flex justify-between items-center border-t border-gray-900 pt-2.5 mb-2">
            ...
            {/* 🟢 الزر أصبح يرسل النموذج مباشرة وبشكل آمن جداً لأن تداخله سليم بنسبة 100% */}
            <button 
              type="submit"
              disabled={loading}
              className="w-full bg-white text-black font-extrabold py-3.5 rounded-xl hover:bg-gray-200 disabled:bg-gray-600 disabled:cursor-not-allowed transition-colors shadow-md cursor-pointer text-center"
            >
